...
def db():
    return sqlite3.connect("/tmp/bot.db")
...
async def delete_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.commit()
    conn.close()
    await query.edit_message_text("🗑️ Задача удалена.", reply_markup=tasks_keyboard(user_id))
...
async def show_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = query.from_user.id
    conn = db()
    total_done = conn.execute("SELECT COUNT(*) FROM tasks WHERE user_id=? AND done=1", (user_id,)).fetchone()[0]
    week_done = conn.execute(
        "SELECT COUNT(*) FROM tasks WHERE user_id=? AND done=1 AND done_at >= date('now', '-7 days')", (user_id,)
    ).fetchone()[0]
    month_done = conn.execute(
        "SELECT COUNT(*) FROM tasks WHERE user_id=? AND done=1 AND done_at >= date('now', '-30 days')", (user_id,)
    ).fetchone()[0]
    active = conn.execute("SELECT COUNT(*) FROM tasks WHERE user_id=? AND done=0", (user_id,)).fetchone()[0]
    avg_mood = conn.execute(
        "SELECT AVG(score) FROM mood WHERE user_id=? AND created_at >= date('now', '-7 days')", (user_id,)
    ).fetchone()[0]
    conn.close()
...
async def focus_mode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = query.from_user.id
    conn = db()
    tasks = conn.execute("SELECT task FROM tasks WHERE user_id=? AND done=0", (user_id,)).fetchall()
    conn.close()
    if not tasks:
        await query.edit_message_text(
